...
// === 🔴 "ЧИСТЕ ПОЛОТНО" (ПУЛЬСУЮЧИЙ АЛЕРТ) ===
      ...
      if (currentAlertStatus.isActive) {
        ...
        // millis() повертає час у мілісекундах.
        // sin() дає плавне коливання від -1 до 1.
        // Ми масштабуємо це до діапазону прозорості (наприклад, від 100 до 255).
        let alphaValue = map(sin(millis() * 0.005), -1, 1, 100, 255); 
        
        fill(255, 0, 0, alphaValue); // Червоний колір з пульсуючою прозорістю
        noStroke();
        textSize(32); // 🔴 Трохи збільшимо розмір тексту для кращої помітності пульсації
        textAlign(CENTER, TOP); // Вирівнювання: центр по горизонталі, верх по вертикалі
        text("🔴AIR ALERT🔴", width / 2, 20); // Малюємо текст по центру з відступом 20px зверху
      }
      // === КІНЕЦЬ ===
}
